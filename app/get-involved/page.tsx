...
export default function GetInvolvedPage() {
  ...
  async function handleVolunteerSubmit(e: React.SyntheticEvent<HTMLFormElement>) {
    ...
    const res = await fetch("/api/volunteer", {
      ...
      body: JSON.stringify({ name: volName, email: volEmail, phone: volPhone, availability, role, notes: volNotes }),
    });
    ...
    if (!res.ok) {
      setVolStatus("error");
      setVolErrorMsg(data.error ?? "Something went wrong.");
    } else {
  ...
  async function handleSponsorSubmit(e: React.SyntheticEvent<HTMLFormElement>) {
    ...
    const res = await fetch("/api/sponsor", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ orgName, contactName, email: sponsorEmail, phone: sponsorPhone, tier, message }),
    });
    ...
    if (!res.ok) {
      ...
      setSponsorErrorMsg(data.error ?? "Something went wrong.");
    } else {
            ...
            {volStatus === "success" ? (
              <div className={styles.successMsg}>
                ...
                <p className={styles.successBody}>
                  Thank you, {volName || "friend"}. Our committee will be in touch
              ...
              <form className={styles.form} onSubmit={handleVolunteerSubmit}>
                ...
                {volStatus === "error" && (
                  <p className={styles.errorMsg}>{volErrorMsg}</p>
                )}
          ...
          <div className={styles.formSection}>
            {sponsorStatus === "success" ? (
              ...
              <form className={styles.form} onSubmit={handleSponsorSubmit}>
                ...
                {sponsorStatus === "error" && (
                  <p className={styles.errorMsg}>{sponsorErrorMsg}</p>
                )}
